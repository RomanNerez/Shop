    					...
    					>
		            		...
		            		<v-row>
		              			...
		              			<v-col cols="12" sm="6" md="6">
		                			<v-text-field
		                				label="Meta Название"
		                				v-model="subCategory.meta_title"
		                				required
		              			...
		              			<v-col cols="12">
	                				<v-select
	                					v-model="subCategory.categories_id"	
	                  					:items="categories"
	                  					label="Категория*"
	                  					item-text="title"
	                  					item-value="id"
	                  					:error-messages="requiredErrors('categories_id')"
	                  					required
	                				></v-select>
	              				</v-col>
		              			<v-col cols="12">
...
<script>
	...
	export default{
		...
		props:['dialog', 'edit', 'selected', 'categories'],
		data: function () {
			return {
				snackbar:{
                    text: '',
		...
		watch: {
			edit: function () {
				this.subCategory = this.edit
				if (!this.edit.file) return; 
				this.subCategory.file = this.location + '/storage/' +this.edit.file
			} 
		},
		mounted: function () {
		...
		methods:{
			getCategories: function () {
				axios.post('/admin/category/getalldata')
				.then(response => {
			...
			requiredErrors: function (key) {
                const errors = []
                if (!this.$v.subCategory[key].$dirty) return errors
                !this.$v.subCategory[key].required && errors.push('Это поле обязательно!')
                return errors
            },
			catchPhoto: function () {
				let that = this;
				...
				window.SetUrl = function (items) {
				    that.subCategory.file = items.map(function (item) {
				    	return item.url;
					}).join(',');
			...
			pushForm() {
				...
				let s = this.selected,
					url = s ? 'admin/sub-categories/edit' : 'admin/sub-categories/add';
				...
				.then(responce =>{
					if (s) {
						for (let i = 0; i < this.$parent.subCategories.length; i++) {
							let subCategory = this.$parent.subCategories[i];
							if (subCategory.id === s) {
								this.$parent.subCategories.splice(i, 1, responce.data);
								break;
							}
						}
					...
					this.$parent.subCategories.unshift(responce.data);
