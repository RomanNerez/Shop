...
<script>
	export default{
        props:['value', 'local', 'status', 'langs', 'valid', 'select', 'tabSelect', 'other'],
		data: function() {
			return {
                alert: {
                    show: false,
        ...
        computed: {
            groups: function () {
                return this.$store.getters.contentData.data.groups.filter(item => {
                    return item.status && item.subs.length && item.categories_id === this.other;
                })
            }
        },
        methods:{
            validate: function () {
                if ( this.value.status ) {
                    if (!Boolean(this.value.subs.length)) {
                        this.alert.show = true;
                        this.alert.text = 'Выберите хотя бы одну рубрику, перед публикацией';
